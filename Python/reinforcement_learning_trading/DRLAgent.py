...
import models.reinforcement_learning.config as config
...
TENSORBOARD_LOG_DIR = '../../models/reinforcement_learning/tensorboard_log'
TRAINED_MODEL_DIR = '../../models/reinforcement_learning/trained_models'
...
def data_split(df, start, end):
    ...
    data = data.sort_values(["Date", "Tickers"], ignore_index=True)
...
class DRLAgent:
    ...
    -------
        train_PPO()
            the implementation for PPO algorith
        train_A2C()
...
#         model = model.learn(total_timesteps=total_timesteps, tb_log_name=tb_log_name)
#         model.save(f"{TRAINED_MODEL_DIR}/{model_name.upper()}_{total_timesteps//1000}k_{iter_num}")
#         return model

#     @staticmethod
#     def get_validation_sharpe(iteration,model_name):
#         ###Calculate Sharpe ratio based on validation results###
#         df_total_value = pd.read_csv('../../models/reinforcement_learning/results/account_value_validation_{}_{}.csv'.format(model_name,iteration))
...
#     def DRL_prediction(self,model,name,last_state,iter_num,initial):
...
#         df_last_state.to_csv('../../models/reinforcement_learning/results/last_state_{}_{}.csv'.format(name, i), index=False)
#         return last_state
